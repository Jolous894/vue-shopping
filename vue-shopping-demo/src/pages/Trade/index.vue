  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in addressList" :key="item.id">
        <span
          class="username"
          :class="{ selected: item.isDefault == 1 ? true : false }"
          >{{ item.consignee }}</span
        >
        <p @click="changeIsDefault(item, addressList)">
          <span class="s1">{{ item.fullAddress }}</span>
          <span class="s2">{{ item.phoneNum }}</span>
          <span class="s3" v-show="item.isDefault == 1 ? true : false"
            >默认地址</span
          >
        </p>
      </div>
      ...
      <div class="detail">
        ...
        >
          ...
          <li>
            <h3>￥{{ order.orderPrice * order.skuNum }}</h3>
          </li>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ orderPriceTotal }}.00</span>
      </div>
    ...
    <div class="sub clearFix">
      <a class="subBtn"  @click="submitOrder"
        >提交订单</a
      >
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...mapState("trade", ["addressList", "orderInfo"]),
    detailArrayList() {
  ...
  methods: {
    ...mapActions("trade", ["getUserAddressList", "getOrderInfo"]),
    //地址切换
    changeIsDefault(item, addressList) {
